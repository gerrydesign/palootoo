...
class ATCF_Campaign {
	...
	public function current_amount( $formatted = true ) {
		...
		foreach ( $prices as $price ) {
			array_push($pal, $price[ 'amount' ]);
		}

		$base = min($pal);
		
		return edd_currency_filter(edd_format_amount( $base ));
	}

	function palootoo_item_limit() {
		$backers = $this->backers();
		$prices  = edd_get_variable_prices( $this->ID );
		$totals  = array();
		$pal = array();
