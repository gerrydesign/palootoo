...
function fundify_customize_register_hero( $wp_customize ) {
	...
	/** RevSlider */
	$wp_customize->add_setting( 'fundify_hero_slider', array(
		'default'    => fundify_theme_mod( 'hero_slider' )
	) );

	$wp_customize->add_control( 'fundify_hero_slider', array(
		'label'      => __( 'Slider Shortcode', 'fundify' ),
		'section'    => 'fundify_hero',
		'settings'   => 'fundify_hero_slider',
		'type'       => 'text',
		'priority'   => 5
	) );
...
function fundify_header_css() {
	$accent_color             = fundify_theme_mod( 'accent_color' );
	$footer_text_color        = fundify_theme_mod( 'footer_text_color' );
	$footer_background_color  = fundify_theme_mod( 'footer_background_color' );
	$footer_background_pos    = fundify_theme_mod( 'footer_background_position' );
	$footer_background_repeat = fundify_theme_mod( 'footer_background_repeat' );
	$footer_background_image  = fundify_theme_mod( 'footer_background_image' );
	...
	$css = "
		...
		#home-page-featured h1 span,
		#title-image h1 span {
			box-shadow: 20px 0 0 $accent_color, -20px 0 0 $accent_color;
		}
	...
	wp_add_inline_style( 'fundify-style', $css );
	...
	if ( $footer_background_image ) {
		$footer_css = "
			#footer {
				...
				background-position-x: $footer_background_pos;
			}";
	} else {
		$footer_css = "
			#footer {
	...
	wp_add_inline_style( 'fundify-style', $footer_css );
}
add_action( 'wp_enqueue_scripts', 'fundify_header_css', 11 );